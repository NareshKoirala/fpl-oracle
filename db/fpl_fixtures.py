...
class FFixtures:
    ...
    def __init__(self, raw_data):
        ...
        if not check:
            ...
            DB.hset_one(f"fpl_fixtures:{self.id}", "deadline_time", str(self.deadline_time))
            ...
            DB.hset_one(f"fpl_fixtures:{self.id}", "data_checked", str(self.data_checked))
            DB.hset_one(f"fpl_fixtures:{self.id}", "most_selected", str(self.most_selected))
            DB.hset_one(f"fpl_fixtures:{self.id}", "highest_score", str(self.highest_score))
            DB.hset_one(f"fpl_fixtures:{self.id}", "most_transferred_in", str(self.most_transferred_in))
            DB.hset_one(f"fpl_fixtures:{self.id}", "top_element", str(self.top_element))
            DB.hset_one(f"fpl_fixtures:{self.id}", "most_captained", str(self.most_captained))
            DB.hset_one(f"fpl_fixtures:{self.id}", "most_vice_captained", str(self.most_vice_captained))
    
    def validate_top_element_info(self):
        temp_dict = {}

        for k, v in self.raw_data["top_element_info"]:
            temp_dict[k] = v
