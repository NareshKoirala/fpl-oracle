...
import { CombinedFixture } from "../data/types";
import { calculateAggregates, getScorelineProb } from "../data/fixtures";
...
export default function FixturesView({ selectedSeason, selectedGW, fixtures }: FixturesViewProps) {
  ...
  const renderFixtureDetailContent = () => {
    ...
    const aggs = calculateAggregates(selectedFixture);
      ...
      <div className="clay-card p-6 border-white/5 relative overflow-hidden text-left">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-white/5 mb-6">
          ...
          <div className="py-1 px-3 bg-white/5 rounded-lg border border-white/5 text-xs font-mono text-white/50 flex items-center gap-1.5 shadow">
            <Calculator className="h-3.5 w-3.5 text-violet-400" /> Over 2.5 Goals: <b className="text-white">{aggs.over25}%</b>
          </div>
        </div>
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-3.5">
                ...
                <div>
                  <div className="flex justify-between text-xs text-white/80 font-medium mb-1.5">
                    <span>{selectedFixture.home_team.name} Win</span>
                    <span className="font-mono text-[#00ff85] font-bold">{aggs.probHome}%</span>
                  </div>
                  <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                    <div style={{ width: `${aggs.probHome}%` }} className="h-full bg-[#00ff85]" />
                  </div>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-xs text-white/80 font-medium mb-1.5">
                    ...
                    <span className="font-mono text-[#7c7c8a] font-bold">{aggs.probDraw}%</span>
                  </div>
                  <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                    <div style={{ width: `${aggs.probDraw}%` }} className="h-full bg-[#7c7c8a]" />
                  </div>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-xs text-white/80 font-medium mb-1.5">
                    <span>{selectedFixture.away_team.name} Win</span>
                    <span className="font-mono text-[#ff005a] font-bold">{aggs.probAway}%</span>
                  </div>
                  <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                    <div style={{ width: `${aggs.probAway}%` }} className="h-full bg-[#ff005a]" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 pt-2">
              <div className="p-3.5 bg-white/[0.01] rounded-2xl border border-white/5">
                <span className="text-[9px] text-[#00ff85] block uppercase font-mono tracking-widest leading-normal">CS Odds: {selectedFixture.home_team.short_name}</span>
                <span className="text-lg font-black text-white font-mono">{aggs.csHome}%</span>
                <p className="text-[10px] text-white/30 mt-1 font-mono">xG: {selectedFixture.stats.xG_home}</p>
              </div>

              <div className="p-3.5 bg-white/[0.01] rounded-2xl border border-white/5">
                <span className="text-[9px] text-[#ff005a] block uppercase font-mono tracking-widest leading-normal">CS Odds: {selectedFixture.away_team.short_name}</span>
                <span className="text-lg font-black text-white font-mono">{aggs.csAway}%</span>
                <p className="text-[10px] text-white/30 mt-1 font-mono">xG: {selectedFixture.stats.xG_away}</p>
              </div>
            </div>
            ...
            <div className="bg-white/5 p-4 rounded-2xl border border-white/5 flex gap-3 text-xs text-white/50">
              ...
              <div>
                Our probability matrices suggest the most expected scoreline for this game is <b>{aggs.maxLabel}</b>, carrying an estimated chance profile of <b>{aggs.maxVal}%</b>.
              </div>
            </div>
          </div>
            ...
            <div className="p-4 bg-white/[0.01] rounded-2xl border border-white/5 space-y-3">
              ...
              <div className="relative">
                ...
                <div className="text-[9px] font-mono font-bold text-center text-white/40 mb-1.5 uppercase letter tracking-widest">
                  Away Goals ({selectedFixture.away_team.short_name}) →
                </div>
                ...
                <div className="flex">
                  ...
                  <div className="text-[9px] font-mono font-bold text-white/40 [writing-mode:vertical-lr] rotate-180 mr-2 flex justify-center uppercase tracking-widest">
                    ← Home Goals ({selectedFixture.home_team.short_name})
                  </div>

                  <div className="grid grid-cols-6 gap-1 w-full flex-1">
                    {[0,1,2,3,4,5].map((homeGoals) => (
                      [0,1,2,3,4,5].map((awayGoals) => {
                        const val = getScorelineProb(selectedFixture, homeGoals, awayGoals);
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="absolute opacity-0 group-hover:opacity-100 bg-[#141416] text-white text-[10px] py-1 px-2 rounded border border-white/10 pointer-events-none transition duration-150 z-50 shadow-xl font-mono -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap">
                              Score {homeGoals}-{awayGoals}: <b className="text-[#00ff85]">{val.toFixed(1)}%</b>
                            </div>
                          </div>
                        )
                      })
              ...
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-white">{f.home_team.name}</span>
                    <span className="text-[10px] text-white/30 font-mono">v</span>
                    <span className="text-sm font-semibold text-white">{f.away_team.name}</span>
                  </div>
                  <div className="flex items-center gap-3 text-[10px] text-white/40 font-mono">
                    <span>H-Win: {aggs.probHome}%</span>
                    <span>•</span>
                    <span>A-Win: {aggs.probAway}%</span>
                  </div>
                </div>
