...
import { getTeamWeek, getActualTeamOfWeek } from "../data/team_week";
import { Fixture, TeamStrength, TeamWeekPlayer } from "../data/types";
...
export default function DashboardView({
  ...
  const strengthList = Array.isArray(teamsStrength) ? teamsStrength : [];

  // Top 3 Attackers & Top 3 Defenders from Strength data
  const topAttackers = [...strengthList]
    .sort((a, b) => b.expected_attack_index - a.expected_attack_index)
    .slice(0, 3);

  const topDefenders = [...strengthList]
    .sort((a, b) => b.expected_defense_index - a.expected_defense_index)
    .slice(0, 3);
              ...
              <div className="clay-card p-6 border-white/5 flex flex-col h-[270px]">
                ...
                <div className="overflow-y-auto flex-1 space-y-3 pr-1">
                  {activeFixtures.slice(0, 4).map((f) => {
                    const maxProb = Math.max(f.prob_home_win, f.prob_draw, f.prob_away_win);
                    let predLabel = "Draw";
                    let predValue = f.prob_draw;
                    let predColor = "text-white/60";
                    if (f.prob_home_win === maxProb) {
                      predLabel = `${f.home_team} Win`;
                      predValue = f.prob_home_win;
                      predColor = "text-[#00ff85]";
                    } else if (f.prob_away_win === maxProb) {
                      predLabel = `${f.away_team} Win`;
                      predValue = f.prob_away_win;
                    ...
                    return (
                      <div key={f.id} className="p-3 bg-white/[0.01] rounded-2xl border border-white/5 hover:border-white/10 transition duration-150">
                        ...
                        <div className="h-2 rounded-full overflow-hidden flex bg-white/5">
                          <div style={{ width: `${f.prob_home_win}%` }} className="stat-bar-win h-full" title={`Home Win: ${f.prob_home_win}%`} />
                          <div style={{ width: `${f.prob_draw}%` }} className="stat-bar-draw h-full" title={`Draw: ${f.prob_draw}%`} />
                          <div style={{ width: `${f.prob_away_win}%` }} className="stat-bar-loss h-full" title={`Away Win: ${f.prob_away_win}%`} />
                        </div>
                      </div>
                    );
                  })}
                    ...
                    <div className="mt-3 space-y-2">
                      {topAttackers.map((t, idx) => (
                        <div key={idx} className="flex justify-between items-center text-xs">
                          <span className="text-white/70 truncate max-w-[80px]">{t.team_name}</span>
                          <span className="font-mono text-[#00ff85] font-bold">{t.expected_attack_index}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="p-3.5 bg-white/[0.01] rounded-2xl border border-white/5">
                    ...
                    <div className="mt-3 space-y-2">
                      {topDefenders.map((t, idx) => (
                        <div key={idx} className="flex justify-between items-center text-xs">
                          <span className="text-white/70 truncate max-w-[80px]">{t.team_name}</span>
                          <span className="font-mono text-violet-400 font-bold">{t.expected_defense_index}</span>
                        </div>
                      ))}
                    </div>
                  </div>
